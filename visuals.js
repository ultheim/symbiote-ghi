...
class Boid {
    constructor(x, y, z, isNewborn = false, burstVel = null) {
        ...
        // NEW: State for individual "cascading" transformation
        this.scanActive = false;
        // Assign random index for snake/scan sync effects
    ...
    update(boids, mouse, width, height, time) {
        ...
        if (window.questionMode && !this.nodeData) {
            ...
            this.targetColor = { r: val, g: val, b: 255 }; 
        ...
        // --- 0.5. DECK/VIDEO MODE: "SEDIMENT" (FALL DOWN) ---
        // If visuals are hidden (Deck Open), particles fall to the bottom and wait.
        if (window.visualsHidden && !this.nodeData) {
             // Define "Floor" relative to screen center (Project coords)
             // Canvas origin (0,0) is center-ish. 
             // Positive Y is down. height * 0.5 is usually bottom of screen relative to center.
             const floorLevel = (height * 0.4); 
             
             // 1. Apply Heavy Gravity
             this.vel.y += 0.8; 
             
             // 2. Air Resistance (Slow down horizontal movement)
             this.vel.x *= 0.90; 
             this.vel.z *= 0.90;  
             
             // 3. Floor Collision (Bounce & Friction)
             if (this.pos.y > floorLevel) {
                 this.pos.y = floorLevel;
                 this.vel.y *= -0.3; // Low bounce (Dampen energy)
                 this.vel.x *= 0.8;  // Friction on the floor
             }
             ...
             // 5. Dim Colors (Sleep Mode)
             this.targetColor = { r: 60, g: 60, b: 60 };
             this.color.r += (this.targetColor.r - this.color.r) * 0.1;
             this.color.g += (this.targetColor.g - this.color.g) * 0.1;
             this.color.b += (this.targetColor.b - this.color.b) * 0.1;
             
             return; // Stop here. Do not flock.
        }

        // --- 0.6. DIRECTOR MODE: SNAKE SWIRL & CASCADE ---
        // EDIT: Added "|| this.scanActive" so they don't lose physics instantly when mode ends
        if ((window.directorMode || this.scanActive) && !this.nodeData) {
            
            // 1. Cascading ON (Wake up)
            if (window.directorMode && !this.scanActive && Math.random() < 0.05) {
                this.scanActive = true;
            }

            // 2. NEW: Cascading OFF (Return to normal)
            // If mode is OFF but I am still active, randomly turn off (5% chance per frame)
            if (!window.directorMode && this.scanActive && Math.random() < 0.05) {
                this.scanActive = false;
            }

            if (this.scanActive) {
                // 1. Cyber Colors (Cyan/Orange)
                this.targetColor = this.index % 2 === 0 
                    ? { r: 0, g: 255, b: 255 }  
                    : { r: 255, g: 100, b: 0 }; 

                // 2. Flatten Z (Screen effect)
                this.pos.z *= 0.95;

                // 3. "Snake Swirl" Motion
                let flowFreq = 0.01;
                let flowSpeed = time * 4.0;
                
                let windX = Math.cos(this.pos.y * flowFreq + flowSpeed);
                let windY = Math.sin(this.pos.x * flowFreq + flowSpeed * 0.5);
                
                this.applyForce({ x: windX * 0.08, y: windY * 0.02, z: 0 }); 
...
window.initSymbiosisAnimation = function() {
    ...
    container.addEventListener('touchend', () => { 
        // Don't kill the vibe instantly. Leave the "attractor" active 
        // for 0.6s so the user sees the boids react to their tap.
        setTimeout(() => {
            rawMouse.active = false; 
            // Note: We do NOT reset rawMouse.x immediately. 
            // Letting coordinates linger creates a smoother "drift away" effect.
        }, 600);
    });
...
// ============================================
// [APPEND] SOCIAL FITNESS VISUALIZER
// Metaphor: "The web of connections"
// ============================================

window.socialModeActive = false;

window.triggerSocialWeb = function() {
    console.log("🕸️ ACTIVATING SOCIAL WEB...");
    window.socialModeActive = true;
    
    // 1. Warm Palette (Gold/Amber representing connection)
    window.curPalette = { 
        pri: {r:255, g:200, b:50},   // Gold
        sec: {r:255, g:140, b:0},    // Amber
        conn: {r:255, g:255, b:255}  // White connections
    };

    // 2. Tighter Physics (Cohesion)
    if(typeof PHYSICS !== 'undefined') {
        PHYSICS.COHESION_WEIGHT = 2.5; // Strong pull together
        PHYSICS.SEPARATION_WEIGHT = 0.5; // Less pushing apart
        PHYSICS.VISION_RAD = 300; // Look further for friends
    }

    // 3. Reset after 15 seconds
    setTimeout(() => {
        window.socialModeActive = false;
        ...
        // Restore Physics
        if(typeof PHYSICS !== 'undefined') {
            PHYSICS.COHESION_WEIGHT = 0.8;
            PHYSICS.SEPARATION_WEIGHT = 2.0;
            PHYSICS.VISION_RAD = 120;
        }
    }, 15000);
};

// [APPEND] POST-RENDER DRAW HOOK (The Web Effect)
// This attempts to hook into the animation loop to draw lines
const _originalReqAnim = window.requestAnimationFrame;
let _canvasContextRef = null;

// Grab context reference when available
window.registerContext = (ctx) => { _canvasContextRef = ctx; };

// Self-running loop to draw connection lines on top of boids
(function socialWebLoop() {
    if (window.socialModeActive && activeGraphBoids.length > 1) {
        ...
        // Draw lines between nearby boids (The "Social Network")
        for (let i = 0; i < activeGraphBoids.length; i++) {
            for (let j = i + 1; j < activeGraphBoids.length; j++) {
                const b1 = activeGraphBoids[i];
                const b2 = activeGraphBoids[j];
                ...
                // If close enough, draw a "relationship"
                if (d < 150) {
                    // Translate to screen center (approximate based on visuals.js logic)
                    const cx = window.innerWidth / 2;
                    const cy = window.innerHeight / 2;
                    
                    ctx.moveTo(cx + b1.pos.x, cy + b1.pos.y);
                    ctx.lineTo(cx + b2.pos.x, cy + b2.pos.y);
                }
            }
        }
        ctx.stroke();
